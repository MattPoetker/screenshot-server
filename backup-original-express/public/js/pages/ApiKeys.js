...
class ApiKeys {
    ...
    async createApiKey(keyData) {
        try {
            ...
            // Add to local array (without raw_key for security)
            const keyForStorage = { ...newKey };
            delete keyForStorage.raw_key;
            this.apiKeys.unshift(keyForStorage);
            
            // Re-render the grid
            this.updateApiKeys(this.apiKeys);
            
            // Show the API key modal with the raw key
            if (window.showApiKeyCreatedModal && newKey.raw_key) {
                window.showApiKeyCreatedModal(newKey);
            } else {
...
// Note: showCreateKeyModal is defined in app.js - don't override it here
